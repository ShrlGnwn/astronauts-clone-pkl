    ...
    <PageShell title="">
      <div className="-mx-4 -mt-10 min-h-screen bg-slate-50">
        <section className="relative w-full overflow-hidden">
          ...
          <div onScroll={handleScroll} className="flex w-full snap-x snap-mandatory overflow-x-auto scrollbar-none">
            {banners.map((banner) => (
          ...
          <div className="absolute bottom-3 inset-x-0 z-20 flex justify-center gap-1.5">
            {banners.map((_, index) => (
              <span key={index} className={`h-2 rounded-full transition-all duration-300 ${
                activeIndex === index
                ? 'w-5 bg-sky-500'
                : 'w-2 bg-white/50'
                }`}
